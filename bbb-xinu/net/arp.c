...
int32 arp_free(int32 slot){
	...
	return OK;
/*
[Note: 	Deleting an entry from  the slot queue is not straightforward. 
	The arp_free function in arp.c file must be leveraged the responsibility of checking a valid deletion operation.
	By valid deletion operation, we imply that no process is waiting for or possessing this arp entry at the time
 	when we delete it from the arpcache, ensuring consistent state of the arpcache.]
*/
}
	...
	)
{
	...
	if (found) {
		...
		if (arptr->arstate == AR_PENDING) {
			...
			/* Inform the waiting process. */
			send(arptr->arpid, OK);
			/* Enqueue the entry */
			
			if(enqueueslot(slot, dlist, 60000) == SYSERR){
				kprintf("\n Cannot enqueue.");
				return;
			}
